...
void *sort_int(void *data)
{
	...
	for (index = 0; index < info->data_length; index += BUF_LEN) {
		...
		if (ret < BUF_LEN) {
			printf("failed to read file, err:%s\n", strerror(errno));
			break;
		}
		printf("read ret:%ld\n", ret);
		for (offset = 0; offset < BUF_LEN; offset+=sizeof(int)) {
			ptr = (int *)(buf + offset);
			//printf("%d\n", *ptr);
			info->hashtable[*ptr]++;
		}
...
int main(int argc, char **argv)
{
	...
	ftruncate(fd2, FILE_LENGTH);

	origin_data = mmap(NULL, FILE_LENGTH, PROT_WRITE, MAP_SHARED, fd2, 0);
	...
	munmap(origin_data, FILE_LENGTH);
	close(fd2);
